...
static void afl_map_obj_conf(afl_t *afl, QDict *json)
{
    ...
    afl->config.qemu.overhead = qnum_get_int(
        afl_conf_obj(json,"qemu-overhead",QTYPE_QNUM,QNum));
    afl->config.qemu.mm_ranges = qstring_get_str(
        afl_conf_obj(json,"vm-mem-ranges",QTYPE_QSTRING,QString));
    afl->config.tgt.fork = qnum_get_uint(
        afl_conf_obj(json,"vm-fork",QTYPE_QNUM,QNum));
    afl->config.tgt.start = qnum_get_uint(
        afl_conf_obj(json,"vm-start",QTYPE_QNUM,QNum));
    afl->config.tgt.end = qnum_get_uint(
        afl_conf_obj(json,"vm-end",QTYPE_QNUM,QNum));
